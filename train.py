...
from utils.train_utils import prepare_output_and_logger, training_report, update_lambdas, scale_grads
# DONE but let's see
...
def training(dataset, opt, pipe, uncertainty_opt, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint):
    ...
    gaussians = GaussianModel(dataset.sh_degree, uncertainty_config=uncertainty_opt, with_mlp=dataset.with_mlp, mlp_W=dataset.mlp_W, mlp_D=dataset.mlp_D, N_a=dataset.N_a)
    ...
    print("\n[Saving Gaussians before training")
    scene.save(0)
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        with torch.no_grad():
            ...
            training_report(tb_writer, iteration, Ll1_unshadowed, Ll1_shadowed, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, render, (pipe, background), 
