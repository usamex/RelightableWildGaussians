...
def create_video_for_envmap(output_path, envs):
    """Create video for environment map visualization"""
...
def render_set(dataset : ModelParams, iteration : int, pipeline : PipelineParams, envmaps, viewpoints):
    ...
    with torch.no_grad():
        dataset.eval = False
        gaussians = GaussianModel(dataset.sh_degree, with_mlp=dataset.with_mlp, 
                                mlp_W=dataset.mlp_W, mlp_D=dataset.mlp_D, N_a=dataset.N_a)
        ...
        for c in cameras:
            ...
            if appearance_idx is not None:
                env_map_list.append(gaussians.compute_env_sh(appearance_idx))
            else:
        ...
        for axis in [0]:  # rotate only around y (changed from 0 to 1 for y-axis)
            ...
            for env_sh_idx, env_sh in enumerate(env_map_list):
                ...
                for angle in tqdm(interp_angle, desc=f"Rotating envmap {env_sh_idx}"):
                    ...
                    rot_env_sh_torch=torch.tensor(rot_env_sh.T, dtype=torch.float32).cuda()
