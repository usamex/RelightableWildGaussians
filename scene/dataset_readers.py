...
def getNerfppNorm(cam_infos: List[CameraInfo]) -> dict:
    def get_center_and_diag(cam_centers: List[np.array]):
...
def storePly(path: str, xyz: np.array, rgb: np.array) -> None:
...
def readColmapSceneInfo(path: str, images: str, eval: bool, eval_file: str, llffhold: int = 8) -> SceneInfo:
    try:
    ...
    images_folder = Path(path) / reading_dir
    ...
    ply_path = Path(path) / "sparse/0" / "points3D.ply"
    bin_path = Path(path) / "sparse/0" / "points3D.bin"
    txt_path = Path(path) / "sparse/0" / "points3D.txt"
